...
namespace IsekaidClass.Isekaid.ClassFeatures
{
    internal class BarbarianFeatures
    {
        ...
        {
            configureMightyRage(enabled: false);
        ...
        public static BlueprintProgression ConfigureEnabled()
        {
            ...
            configureMightyRage(enabled: true);
        ...
        private static void configureMightyRage(bool enabled)
        {
            logger.Info("   Configuring mighty rage");

            string name = "MightyRage";

            if (!enabled)
            {
                FeatureConfigurator.New(name, Guids.MightyRage).Configure();
                return;
            }

            FeatureConfigurator.New(name, Guids.MightyRage)
            ...
            BuffConfigurator.For(BuffRefs.StandartRageBuff.Reference.Get())
                ...
                .EditComponent<ContextRankConfig>(
                    edit: c => c.m_FeatureList = CommonTool.Append(
                        c.m_FeatureList,
                        BlueprintTool.GetRef<BlueprintFeatureReference>(Guids.MightyRage)
                    )
                )
                .Configure();
        }
